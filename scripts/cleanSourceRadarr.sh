...
# Instructions
# Put this script somewhere on your file system and make it executable.
#
...
# On Upgrade: Yes
# On Rename: No
# Path: /path/to/script/cleanSourceRadarr.sh
# Arguments:
...
torrent_path_portion="downloads"

# Test that this is a download event, so we don't run on grab or rename.
# shellcheck disable=SC2154
if [[ "${radarr_eventtype}" != "Download" ]]; then
  echo "[Torrent Cleanup] Sonarr Event Type is NOT Download, exiting." | tee cleanSourceRadarr.log -a
  exit
fi
...
if ! [[ "${radarr_moviefile_sourcepath}" =~ ${torrent_path_portion} ]]; then
  echo "[Torrent Cleanup] Path ${radarr_moviefile_sourcepath} does not contain \"${torrent_path_portion}\", exiting."
  exit
fi
