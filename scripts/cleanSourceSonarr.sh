...
# On Upgrade: Yes
# On Rename: No
# Path: /path/to/script/cleanSourceSonarr.sh
# Arguments:
...
sonarr_final_dir="downloads"

# Identifiable portion of path to torrents, so it will only run on torrents.
# For example, with a path of "/data/torrents/tv", "torrents" is a good choice.
torrent_path_portion="downloads"

# Test that this is a download event, so we don't run on grab or rename.
# shellcheck disable=SC2154
if [[ "${sonarr_eventtype}" != "Download" ]]; then
  echo "[Torrent Cleanup] Sonarr Event Type is NOT Download, exiting." | tee cleanSourceSonarr.log -a
  exit
fi
